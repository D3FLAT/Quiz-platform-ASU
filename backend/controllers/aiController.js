...
ВАЖНО — ЗАПРЕЩЕНО:
- Отвечать на вопросы из квизов напрямую (если пользователь спрашивает "какой ответ на вопрос X" или "что правильно выбрать" — вежливо откажи и предложи подумать самостоятельно)
- Решать задания и тесты за пользователя
...
exports.chat = async (req, res) => {
  const { message, history = [], page = 'неизвестна' } = req.body;
  ...
  try {
    const messages = [
      { role: 'system', content: SYSTEM_PROMPT + `\n\nТекущая страница пользователя: ${page}.`},
      ...history.slice(-6),
